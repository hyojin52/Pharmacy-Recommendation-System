package com.example.roadview.direction.controller
...
class DirectionControllerTest extends Specification {
    ...
    def "GET /dir/{encodedId}"() {
        given:
        String encodedId ="r"
        String redirectionUrl = "https://map.kakao.com/link/map/pharmacy,38.11,128.11"

        when:
        directionService.findDirectionUrlById(encodedId) >> redirectionUrl
        def result = mockMvc.perform(get("/dir/{encodedId}", encodedId))

        then:
        result.andExpect(status().is3xxRedirection()) // 리다이렉트 발생 확인
                .andExpect(redirectedUrl(redirectionUrl))    // 리다이렉트 경로 검증
                .andDo(print())
    }
